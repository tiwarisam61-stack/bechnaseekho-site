...
app.get('/jobs/:jobId', (_req, res) => {
    res.sendFile(path.join(frontendDir, 'job-details.html'));
});

app.get('/my-applications', (_req, res) => {
    res.sendFile(path.join(frontendDir, 'my-applications.html'));
});

app.get('/admin/applications', (_req, res) => {
    res.sendFile(path.join(frontendDir, 'admin-applications.html'));
});
...
app.delete('/api/applications/:applicationId', async (req, res) => {
    ...
    if (result.appToDelete?.resume?.path) {
        const absoluteResumePath = path.join(backendDir, result.appToDelete.resume.path.replace(/^\//, ''));
        try {
