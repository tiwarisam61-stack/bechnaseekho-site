...
function pageFooter(doc: Doc, page: number, title: string) {
  ...
  doc.setFontSize(8);
  doc.setTextColor(...SLATE);
  doc.text(`CareerSync by BechnaSeekho  •  ${title}`, M, H - 11);
...
export async function generatePdf(kind: BrochureKind, courses: Course[], course?: Course) {
  ...
  const titles: Record<BrochureKind, string> = {
    course: course ? `${course.title} — Syllabus` : "Course Syllabus",
  ...
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(8.5);
  doc.text("careersync.bechnaseekho.com  •  hello@bechnaseekho.com  •  +91 90000 00000", M, H - 9);
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(9);
  doc.text("CAREERSYNC  •  " + title.toUpperCase(), M, 7.8);
  ...
  doc.setTextColor(203, 213, 225);
  doc.text(
    `${inr(meta.originalPrice)} original  •  ${meta.discount}% off  •  EMI from ${inr(meta.emi)}/month`,
    M + 8,
    y + 23,
  );
  doc.setFillColor(...GOLD);
  ...
  doc.setTextColor(...SLATE);
  doc.text(
    "hello@bechnaseekho.com   •   +91 90000 00000   •   careersync.bechnaseekho.com",
    M,
    y,
  );
