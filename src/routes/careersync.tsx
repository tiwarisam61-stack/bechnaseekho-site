...
export const Route = createFileRoute("/careersync")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "CareerSync by BechnaSeekho: discover jobs, build skills with resume tools and mock interviews, and land your dream role." },
      { property: "og:title", content: "CareerSync — Find Jobs, Build Skills" },
      { property: "og:description", content: "Career platform. Jobs, resume builder, interview prep — all in one place." },
...
type NavLink = {
  label: string;
  href?: string;
  to?: "/careersync" | "/careersync/about" | "/careersync/blogs" | "/careersync/verified-recruiters" | "/resources" | "/pricing" | "/ats-score-checker" | "/resume-templates" | "/careersync-academy";
  newTab?: boolean;
  children?: {
    label: string;
    desc: string;
    to?: "/resources" | "/pricing" | "/ats-score-checker" | "/resume-templates";
    upcoming?: boolean;
  }[];
};
const NAV_LINKS: NavLink[] = [
  ...
  { label: "Jobs", href: "#jobs" },
  {
    label: "AI Arena",
    children: [
      { label: "ATS Score Checker", desc: "Check and improve your resume match score", to: "/ats-score-checker" },
      { label: "Resume Templates", desc: "ATS-friendly resume templates for every role", to: "/resume-templates" },
      { label: "Salary Calculator", desc: "Estimate your salary by role, experience and location", upcoming: true },
    ],
  },
    ...
    >
      ...
      <div className="mx-auto max-w-[1400px]">
        ...
        >
          ...
          {/* Logo */}
          <Link to="/" className="relative flex items-center gap-2.5 pl-2 pr-3 shrink-0 group" aria-label="Back to BechnaSeekho home">
            <motion.img src="/favicon.png" alt="BechnaSeekho logo" width={40} height={40} whileHover={{ rotate: 4, scale: 1.05 }} transition={{ type: "spring", stiffness: 300 }} className="h-10 w-10 object-contain drop-shadow-[0_8px_20px_rgba(37,99,235,0.35)]" draggable={false} />
            <div className="leading-tight">
              <div className="font-extrabold text-[19px] tracking-tight" style={{ fontFamily: "'Sora','Inter',ui-sans-serif,system-ui" }}>
                <span style={{ color: "#0F172A" }}>Career</span><span style={{ backgroundImage: "linear-gradient(135deg,#2563EB 0%,#3B82F6 55%,#06B6D4 100%)", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>Sync</span>
              </div>
          ...
          {/* Center nav */}
          <nav className={`relative hidden min-w-0 items-center rounded-full bg-white/60 p-1 ring-1 ring-blue-100/70 lg:flex ${isAuthenticated ? "gap-0" : "gap-0.5"}`}>
            {NAV_LINKS.map((l) => {
              const isActive = active === l.label;
              const hasMenu = !!l.children?.length;
              const isOpen = openMenu === l.label;
              const cls = `relative flex items-center gap-1 whitespace-nowrap rounded-full py-2 transition-colors ${isAuthenticated ? "px-3 text-sm font-medium" : "px-4 text-[15px] font-semibold"} ${isActive ? "text-[#1a2a4a]" : "text-gray-500 hover:text-[#1a2a4a]"
                }`;
              const inner = (
                <>
                  {isActive && (
                  ...
                  {hasMenu && (
                    <ChevronDown className={`relative z-10 h-3.5 w-3.5 transition-transform ${isOpen ? "rotate-180" : ""}`} />
                  )}
                </>
              );
              if (hasMenu) {
                        ...
                        >
                          <div className="overflow-hidden rounded-2xl bg-white p-2 shadow-2xl ring-1 ring-black/5">
                            {l.children!.map((item) => {
                              ...
                              const content = (
                                <>
                                  <div className={`mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-lg ${iconWrapClass}`}>
                                    {iconNode}
                                  </div>
                                  <div>
        ...
        {mobileOpen && (
          ...
          >
            <div className="flex flex-col gap-1">
              {NAV_LINKS.map((l) => {
                const hasMenu = !!l.children?.length;
                const cls = `rounded-2xl px-4 py-3 text-sm font-semibold ${active === l.label ? "bg-blue-50 text-blue-600" : "text-[#1a2a4a] hover:bg-blue-50/60"
                  }`;
                if (hasMenu) {
                  return (
                    <div key={l.label} className="rounded-2xl bg-white/70 ring-1 ring-blue-100">
                      ...
                      <div className="px-2 pb-2">
                        {l.children!.map((item) => {
                          ...
                          const content = (
                            <>
                              <div className={`mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-lg ${iconWrapClass}`}>
                                {iconNode}
                              </div>
                              <div>
                                <div className="flex items-center gap-2 text-sm font-bold text-[#1a2a4a]">
                ...
                return l.to ? (
                  ...
                  <a key={l.label} href={l.href} onClick={() => { setActive(l.label); setMobileOpen(false); }} className={cls}>
                    {l.label}
                  </a>
                );
              })}
...
function PostJobNavButton() {
  ...
  return (
    ...
    >
      <Briefcase className={user ? "h-3 w-3 text-blue-600" : "h-3.5 w-3.5 text-blue-600"} /> Post a Job
    </Link>
  );
}
...
function AuthArea() {
  ...
  if (user) {
    const displayName = (user.user_metadata?.full_name as string | undefined)?.trim() || user.email?.split("@")[0] || "CareerSync user";
    ...
    return (
      <div className="mt-2 space-y-2">
        ...
        <div className="flex items-center justify-between rounded-full bg-blue-50 px-4 py-2.5 text-sm font-semibold text-[#1a2a4a]">
          <span className="min-w-0 truncate">{displayName}{roleLabel ? ` · ${roleLabel}` : ""}</span>
          <button onClick={() => { void signOut(); onClose(); }} className="text-gray-500 hover:text-[#1a2a4a]">
            Log out
          </button>
        </div>
      </div>
  ...
  return (
    <div className="mt-2 space-y-2">
      ...
      <div className="grid grid-cols-2 gap-2">
        <Link to="/login" search={{ role: "candidate" }} onClick={onClose}
        ...
        <Link to="/login" search={{ role: "company" }} onClick={onClose}
        ...
        <Link to="/login" search={{ role: "employee" }} onClick={onClose}
        ...
        <Link to="/signup" search={{ role: "candidate" }} onClick={onClose}
        ...
        <Link to="/signup" search={{ role: "company" }} onClick={onClose}
        ...
        <Link to="/signup" search={{ role: "employee" }} onClick={onClose}
...
function CareerSyncPage() {
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      {user && role && (
        <section className="mx-auto max-w-7xl px-6 pt-2">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-blue-100 bg-blue-50/70 p-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-wide text-blue-700">Signed in as {role}</p>
              <p className="text-sm text-slate-700">You are on the public CareerSync landing page. Open your workspace when you are ready.</p>
            </div>
            <button
              type="button"
      ...
      {/* HERO */}
      <section id="home" className="max-w-7xl mx-auto px-6 pt-10 pb-4 grid lg:grid-cols-2 gap-10 items-center min-h-[82vh] scroll-mt-24">
        <div className="flex flex-col gap-6">
          ...
          <div className="space-y-1">
            <h1 className="text-5xl xl:text-6xl font-black text-[#1a2a4a] leading-tight">Find Jobs.</h1>
            <h1 className="text-5xl xl:text-6xl font-black text-[#1a2a4a] leading-tight">Build Skills.</h1>
          ...
          <p className="text-gray-500 text-lg leading-relaxed max-w-md">
            Discover the right opportunities, enhance your skills,<br className="hidden sm:block" />
          ...
          <div className="flex flex-wrap gap-4">
            ...
            >
              <Upload className="h-5 w-5 text-blue-600 transition-transform group-hover:scale-110" /> Upload Resume
            </button>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-2">
            {[
              { icon: <Briefcase className="w-5 h-5 text-blue-600" />, label: "10K+", sub: "Active Jobs" },
              { icon: <FileText className="w-5 h-5 text-blue-600" />, label: "Resume", sub: "Builder" },
              { icon: <Star className="w-5 h-5 text-blue-600" />, label: "Mock", sub: "Interviews" },
              { icon: <Users className="w-5 h-5 text-blue-600" />, label: "Expert", sub: "Guidance" },
            ].map(({ icon, label, sub }) => (
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 sm:px-6 py-20">
      <div className="text-center">
        <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-blue-600">Career Categories</p>
        <h2 className="mt-3 text-3xl sm:text-4xl font-black tracking-tight text-[#0F172A]">Explore roles by domain</h2>
      </div>
      <div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {categories.map((c) => (
          <div key={c.title} className="rounded-2xl border border-blue-100 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
            <div className="text-sm font-black text-[#0F172A]">{c.title}</div>
            <p className="mt-2 text-sm text-slate-600">{c.desc}</p>
            <div className="mt-4 inline-flex rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">{c.jobs} jobs</div>
          </div>
        ))}
      </div>
    </section>
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 sm:px-6 py-20">
      <div className="flex items-end justify-between gap-4">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-blue-600">Blogs Preview</p>
          <h2 className="mt-2 text-3xl sm:text-4xl font-black tracking-tight text-[#0F172A]">Learn from expert playbooks</h2>
        </div>
        <Link to="/careersync/blogs" className="text-sm font-semibold text-blue-700 hover:text-blue-800">View all blogs</Link>
      </div>
      <div className="mt-8 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        {BLOGS.slice(0, 3).map((blog) => (
          <Link key={blog.slug} to="/careersync/blogs/$slug" params={{ slug: blog.slug }} className="group rounded-2xl border border-blue-100 bg-white p-4 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
            <img src={blog.cover} alt={blog.title} loading="lazy" className="aspect-[16/10] w-full rounded-xl object-cover" />
            <p className="mt-3 text-[11px] font-bold uppercase tracking-wide text-blue-600">{blog.category}</p>
            <h3 className="mt-1 text-base font-bold text-[#0F172A] group-hover:text-blue-700 line-clamp-2">{blog.title}</h3>
...
function ResumeUploadModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const [form, setForm] = useState<ResumeFormData>({ fullName: "", email: "", phone: "", resume: null });
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    // TODO: Integrate with ATS pipeline API and resume database.
    // TODO: Add company email/WhatsApp notifications after backend integration.
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-900/55 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-xl rounded-3xl border border-blue-100 bg-white p-6 shadow-2xl sm:p-7" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <h2 className="text-2xl font-black tracking-tight text-[#0F172A]">Upload Resume</h2>
            <p className="mt-1 text-sm text-slate-600">Share your profile and we will review it for suitable opportunities.</p>
          </div>
          <button onClick={onClose} className="grid h-9 w-9 place-items-center rounded-full text-slate-500 transition hover:bg-slate-100 hover:text-slate-800" aria-label="Close">
        ...
        {showSuccess ? (
          <div className="grid place-items-center py-10 text-center">
            <motion.div initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="grid h-16 w-16 place-items-center rounded-full bg-emerald-100 text-emerald-600">
              <CheckCircle2 className="h-8 w-8" />
            </motion.div>
            <p className="mt-4 text-lg font-bold text-slate-900">Resume submitted successfully.</p>
            <p className="mt-1 text-sm text-slate-600">Thank you. Our team will review your profile.</p>
          </div>
        ) : (
          <form onSubmit={onSubmit} className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="sm:col-span-2">
                ...
                <input value={form.fullName} onChange={(e) => setField("fullName", e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100" placeholder="Your full name" />
              ...
              <div>
                ...
                <input type="email" value={form.email} onChange={(e) => setField("email", e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100" placeholder="you@example.com" />
                {errors.email && <p className="mt-1 text-xs text-rose-600">{errors.email}</p>}
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500">Phone Number</label>
                <input value={form.phone} onChange={(e) => setField("phone", e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100" placeholder="+91 9876543210" />
            ...
            <div>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-slate-800">{form.resume ? form.resume.name : "Drag & drop your resume here"}</p>
                ...
                <input
                  ...
                  className="hidden"
                  onChange={(e: ChangeEvent<HTMLInputElement>) => onFilePick(e.target.files?.[0] ?? null)}
                />
              </label>
            ...
            <div className="flex items-center justify-end gap-2 pt-1">
              <button type="button" onClick={onClose} className="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Cancel</button>
              <button type="submit" disabled={submitting} className="inline-flex items-center gap-2 rounded-full bg-blue-600 px-5 py-2 text-sm font-bold text-white transition hover:bg-blue-700 disabled:opacity-60">
