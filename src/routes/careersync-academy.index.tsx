...
import { courses, type Course } from "@/lib/academy-courses";
import { courseMeta } from "@/lib/academy-course-meta";
import { useAllAcademyStats, useAcademyProgress } from "@/lib/academy-progress";
import { CourseCard } from "@/components/academy/course-card";
import { CourseGridSkeleton } from "@/components/academy/course-skeleton";
import { CourseDetailModal } from "@/components/academy/course-detail-modal";
import { Reveal, CountUp } from "@/components/academy/reveal";
import {
  RoadmapSection,
  CategoriesSection,
  FeaturesSection,
...
export const Route = createFileRoute("/careersync-academy/")({
  head: () => ({
    meta: [
      { title: "Premium Career Courses — Build Job-Ready Skills | CareerSync Academy" },
      {
        name: "description",
        content:
...
const durationOptions = ["All", "Under 6h", "6–8h", "8h+"] as const;
const languageOptions = ["All", "English", "English + Hindi", "Hindi"] as const;
const certOptions = ["All", "Certificate", "Live Sessions"] as const;
const priceOptions = ["All", "Free"] as const;
const sortOptions = ["Most Popular", "Highest Rated", "Price: Low to High", "Price: High to Low", "Newest"] as const;
...
function Catalog() {
  const all = useAllAcademyStats();
  const { store, hydrated } = useAcademyProgress();
  ...
  const download = async (kind: BrochureKind, course?: Course) => {
    ...
    try {
      const { generatePdf } = await import("@/lib/academy-brochure");
